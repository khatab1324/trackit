...
export function ProfileScreen() {
  ...
  const [getCurrentUserMemories, { isLoading, isError }] = useGetCurrentUserMemoriesMutation();

  useEffect(() => {
    const unsubscribe = navigation.addListener("focus", async () => {
      try {
        const result = await getCurrentUserMemories();
        const data = result?.data?.data as Memory[] || [];
        setMemories(data);
      } catch (error) {
  ...
  if (isLoading) {
    content = <View className="flex-1 justify-center items-center">
      <ActivityIndicator size="large" />
    </View>
  } else if (isError) {
    content = <View className="flex-1 justify-center items-center">
      <Text style={{ color: colorScheme.error }}>Error fetching memories</Text>
    </View>
  }
  ...
  return (
    <View className={`flex-1 px-4 pt-12`} style={styles.container}>
      ...
      <View className="flex-row justify-between items-center mb-2">
        <Text className={`text-2xl font-bold`} style={styles.text}>Profile</Text>
        <TouchableOpacity onPress={() => navigation.navigate("Settings")}>
          <Ionicons name="settings-outline" size={24} color={colorScheme.icon.secondary} />
        </TouchableOpacity>
      </View>
      ...
      <View className="flex-row justify-around mt-6 mb-10">
        <View className="items-center">
          <Text className={`text-lg font-bold text-red-400`} style={styles.text}>
            {memories.length || 0}
          </Text>
